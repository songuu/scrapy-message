...
class TestDBHelper():
    ...
    def testCreateTable(self):
        sql="create table suse_info(id int primary key auto_increment,name varchar(50),url varchar(200))"
        self.dbHelper.createTable(sql)
    #测试插入
    def testInsert(self):
        sql="insert into suse_info(name,url) values(%s,%s)"
        params=("test","test")
        self.dbHelper.insert(sql,*params) #  *表示拆分元组，调用insert（*params）会重组成元组
    def testUpdate(self):
        sql="update suse_info set name=%s,url=%s where id=%s"
        params=("update","update","1")
        self.dbHelper.update(sql,*params)
    
    def testDelete(self):
        sql="delete from suse_info where id=%s"
        params=("1")
